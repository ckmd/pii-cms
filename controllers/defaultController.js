...
module.exports = {
    index : async(req,res) => {
        ...
        const rubix3 = await Post.find({setAsRubix:true, status:'Events-others'}).populate('category');
        const rubix4 = await Post.find({setAsRubix:true, status:'Info'}).populate('category');
        const rubix5 = await Post.find({setAsRubix:true, status:'Events-others'}).populate('category');
